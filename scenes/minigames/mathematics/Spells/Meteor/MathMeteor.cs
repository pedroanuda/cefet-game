		...
		private void OnHalfEnded(StringName animName) {
			switch (animName) {
				case "default":
					...
					// Explosion Animation
					var explosionSprite = GetNode<AnimatedSprite2D>("Core/ExplosionSprite");
					explosionSprite.AnimationFinished += explosionSprite.QueueFree;
					explosionSprite.Reparent(GetParent());
